...
A digital image is just a giant grid of tiny colored squares called **pixels**.

::: {.incremental}
- **Location**: the exact row and column position of the pixel in the image
- **Grayscale images (black & white)**: each pixel has only ONE value, from **0 (pure black)** to **255 (pure white)**
- **Colored images (RGB)**: each pixel is made by mixing THREE main colors
  - **R** = Red
  - **G** = Green
  - **B** = Blue
:::
...
::: {.sub}
By mixing different intensities of these 3 colors (0 to 255 each), the computer can create **millions of colors**! 🎨
:::
...
## How Data is Used in Training Computer Vision Models? {.smaller}

::: {.incremental}
1. **Feed**: give the model thousands of images labeled "cat" and "dog"
2. **Learn**: the model automatically learns patterns and differences between cats and dogs
3. **Test**: show the model new images → test if it can correctly classify them
:::
...
::: {.incremental}
- **Neuron** (the decision maker): a tiny unit that takes input numbers, decides how important they are by multiplying **weights**, then passes the result on to the next step
- **Layer** (the team): a group of neurons working together, at the same time, looking at the same data
- **Network** (the whole chain): stacks of layers working in order , the output of Team 1 becomes the input of Team 2
:::

::: {.sub}
**"Deep"** simply means we have MANY teams (layers) working together!
:::

![](assets/day3/nn_vs_deep.png){fig-align="center" height="230"}
...
::: {.incremental}
1. **Predict**: the network guesses what is in the image
2. **Check error**: a **loss function** measures how wrong that guess was
3. **Fix mistakes**: **backpropagation** adjusts the network to reduce the error , it learns from its mistakes, and repeats **millions of times** until it gets it right
:::
...
::: {.incremental}
- **Early layers**: see simple patterns , edges and lines
- **Deeper layers**: combine lines , see corners and shapes
- **Final layers**: combine shapes , see whole objects, like cars or faces
:::
